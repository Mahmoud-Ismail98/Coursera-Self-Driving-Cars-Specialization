...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_hat = Quaternion(euler=del_qk).quat_mult_right(q_check) 
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2/2)*(Cns@imu_f.data[k-1] + g) 
    ...
    F[3:6, 6:] = -(Cns.dot(skew_symmetric(imu_f.data[k-1].reshape((3,1)))))*delta_t 
    Q = np.eye(6)                                                              
    Q[0:3,0:3] *= delta_t**2*var_imu_f                                         
    Q[3:6,3:6] *= delta_t**2*var_imu_w                                         
